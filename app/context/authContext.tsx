...
import { OrganizationProfile, USER_ROLE, UserProfile } from "../typings/profile/typings";
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [organizations, setOrganizations] = useState<OrganizationProfile[] | null>(null);
      ...
      if (status === "authenticated" && session?.user?.email) {
        try {
          ...
          if (userProfile) {
            ...
            if (userProfile.organization && userProfile.organization.length > 0) {
              setIsOrganizer(true);
              localStorage.setItem('userRole', "Organizer");
              
              // Fetch organizations data
              const organizationPromises = userProfile.organization.map((orgId: string) => 
                getOrganizationProfile(orgId)
              );
              
              const orgProfiles = await Promise.all(organizationPromises);
              const validOrgProfiles = orgProfiles.filter(profile => profile !== null) as OrganizationProfile[];
              setOrganizations(validOrgProfiles);
              localStorage.setItem('organizationData', JSON.stringify(validOrgProfiles));
            } else {
              setIsOrganizer(false);
              localStorage.removeItem('userRole');
              localStorage.removeItem('organizationData');
  ...
  return (
    <AuthContext.Provider value={{ user, organizations, isOrganizer, isLoading, clearUser }}>
      {children}
